...
```mysql
select * from student where name="cxuan" for updateCOPY
```
...
![img](https://gitee.com/Cai_Programmer/pic-go2/raw/master/93c182cd-4b58-4a47-8be9-fa7770564cab.jpg)
...
![img](https://gitee.com/Cai_Programmer/pic-go2/raw/master/65f16b0c-3d88-4c5d-8b81-fa03600002bd.jpg)

- 事务一开启，男柜员先执行读操作，取出金额和版本号，执行写操作

  ```mysql
  begin
  update 表 set 金额 = 120,version = version + 1 where 金额 = 100 and version = 0COPY
  ```
  ...
  ```mysql
  begin
  update 表 set 金额 = 50,version = version + 1 where 金额 = 100 and version = 0COPY
  ```
